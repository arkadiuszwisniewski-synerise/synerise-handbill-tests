...
- **POS flows** — price a basket against the profile's promotions ([processSale](https://hub.synerise.com/api-reference/loyalty-and-engagement#tag/Promotions/operation/processSale_POST)) and issue the coupons printed on the receipt ([processCheckout](https://hub.synerise.com/api-reference/loyalty-and-engagement#tag/Promotions/operation/processCheckout_POST)), from an editable JSON payload with three ready-made templates (sale, checkout, and a sale carrying a scanned coupon). Neither call redeems a promotion or creates a transaction. The result is read from `transactionItems[].grantedDiscounts[]` — `discountOrigin: "PROMOTION"` or `"COUPON"`; the `transactionAdditionalItems[].status` field reports `"INVALID"` even for coupons that applied, so the section's tooltips spell out what to trust.
